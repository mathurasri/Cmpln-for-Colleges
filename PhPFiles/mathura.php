		...
		if (mysql_num_rows($result) > 0)
		{
			...
			if($imageuri != null)
			{
				...
				if ($result) {        
					...
					$response["success"] = 0;
					$response["message"] = "Oops! An error in imageuri1 occurred.";         
					echo json_encode($response);
				}
			}
			else
			{
				$response["success"] = 0;
					$response["message"] = "Oops! An error occurred in imageuri2.";         
					echo json_encode($response);
			}
			if($username != null)
			{
				...
				if ($result) {        
					$response["success"] = 1;
					$response["message"] = "UserProfile successfully  inserted in username.";        
					echo json_encode($response);
				} else {        
					$response["success"] = 0;
					$response["message"] = "Oops! An error occurred in username1.";         
					echo json_encode($response);
				}
			}
			else
			{
				$response["success"] = 0;
					$response["message"] = "Oops! An error occurred in username2.";         
					echo json_encode($response);
			}
			if($bio != null)
			{
				...
				if ($result) {        
					$response["success"] = 1;
					$response["message"] = "UserProfile successfully  inserted. in bio";        
					echo json_encode($response);
				} else {        
					$response["success"] = 0;
					$response["message"] = "Oops! An error occurred in bio1.";         
					echo json_encode($response);
				}
			}
			else
			{
				$response["success"] = 0;
					$response["message"] = "Oops! An error occurred in bio2.";         
					echo json_encode($response);
			}
			//echo "A row exists";
		}

...
<?php
//This program downloads the comments of the people who follow you.
$response = array();
if (isset($_POST['university'] ) && isset($_POST['start'] ) && isset($_POST['count'])&& isset($_POST['username']))
{
	...
	$sql = "select count(*) as likesCount, table3.comment_id,table3.comment, table3.avatarPosition, table3.username, table3.server_time_posted, table3.likecommentid, table3.time_posted from (select * from ((select comments.comment_id, comments.time_posted as time_posted, comments.comment, UserProfile.avatarPosition, comments.username, comments.server_time_posted from comments, UserProfile where comments.email = UserProfile.email and comments.university = '$university' and comments.username in (select followed_by from follows where following='$username')) as table1 left outer join (select likes.comment_id as likecommentid from likes) as table2 on table1.comment_id=table2.likecommentid) ) as table3 group by table3.comment_id, table3.avatarPosition, table3.username, table3.server_time_posted, table3.likecommentid, table3.comment, table3.time_posted order by table3.time_posted desc LIMIT $start, $count";
	...
	if(!empty($retval))
	{
		...
		{
			$response["success"] = 0;
			$response["message"] = "Could not the follower comments";
	...
	{
		$response["success"] = 0;
			$response["message"] = "Could not download the follower comments";			
			echo json_encode($response);
			//echo "No rows found";
	}
